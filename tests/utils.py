...
def get_send_elements(agr_obj, is_server, opcode=e.IBV_WR_SEND):
    ...
    """
    if opcode == e.IBV_QP_EX_WITH_RDMA_ATOMIC_WRITE:
...
def post_send_ex(agr_obj, send_object, send_op=None, qp_idx=0, ah=None):
    ...
    elif send_op == e.IBV_QP_EX_WITH_RDMA_ATOMIC_WRITE:
        qp.wr_rdma_atomic_write(agr_obj.rkey, agr_obj.raddr, send_object)
    elif send_op == e.IBV_QP_EX_WITH_RDMA_FLUSH:
        qp.wr_rdma_flush(agr_obj.rkey, agr_obj.raddr, agr_obj.msg_size,
                         agr_obj.type, agr_obj.level)
    ...
    if send_op != e.IBV_QP_EX_WITH_RDMA_ATOMIC_WRITE and \
            send_op != e.IBV_QP_EX_WITH_RDMA_FLUSH:
...
def rdma_flush_traffic(client, server, iters, gid_idx, port, new_send=False,
                       send_op=None):
    """
