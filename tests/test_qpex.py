...
class QpExTestCase(RDMATestCase):
    ...
    def test_qp_ex_rc_rdma_flush(self):
        ...
        wcs = u.rdma_flush_traffic(client, server, self.iters, self.gid_index, self.ib_port,
    ...
    def test_qp_ex_rc_rdma_flush_type_violate(self):
        client, server = self.create_players('rc_flush')
        ...
        client.raddr = server.mr.buf
        server.raddr = client.mr.buf
        client.type = e.IB_FLUSH_PERSISTENT
        wcs = u.rdma_flush_traffic(client, server, self.iters, self.gid_index, self.ib_port,
                       new_send=True, send_op=e.IBV_QP_EX_WITH_RDMA_FLUSH)
